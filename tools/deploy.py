...
def secret():return re.search(r'^ACCOUNT_1_GENLAYER_PRIVATE_KEY\s*=\s*"?([^"\r\n]+)',ENV,re.M).group(1).strip()
def address(value):
...
def main():
 ...
 code=(ROOT/"src/recall.py").read_text()
 ...
 (ROOT/"casebook/deployment.json").parent.mkdir(parents=True,exist_ok=True)
 (ROOT/"casebook/deployment.json").write_text(json.dumps(out,indent=2))
